...
type GenerateConsensusPageMetaParams = { t: (key: string) => string; topic?: TTopic; version?: TVersion };

export function generateConsensusPageMeta({ t, topic, version }: GenerateConsensusPageMetaParams): Metadata {
  let title = t(`title`);
  let description = '';
